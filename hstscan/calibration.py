    ...
    if False:
        ...
        for i in range(len(TRACE)):
            if abs(YREF - i) < 0.5:
                ...
                p.plot(x[0,int(XREF)+BEAM_X1:int(XREF)+BEAM_X2], YREF+TRACE[i][int(XREF)+BEAM_X1:int(XREF)+BEAM_X2], color='k')
        ...
        if original_image is None: view(trace_l, title='Position along trace', cmap='hot')
        else: view(original_image)
    if False:
        # Just plot the trace locally
        i = np.round(YREF)
        p.plot(x[0,int(XREF)+BEAM_X1:int(XREF)+BEAM_X2], YREF+TRACE[i][int(XREF)+BEAM_X1:int(XREF)+BEAM_X2], color='k', label='Trace')
        p.plot(x[0,int(XREF)+BEAM_X1:int(XREF)+BEAM_X2], [YREF]*(BEAM_X2-BEAM_X1), color='k', ls='--')
