...
import numpy as np
import re, time, os, subprocess, calendar
...
def suntimecorr(ra, dec, obst,  coordtable, verbose=False):
  """
    ...
    !
     ...
     CENTER     = '@0'                 ! Set observer (coordinate center)
    ...
    2016-02-22 jarcang  removed splinterp dependency
  ...
  '''
  # Using splinterp (if available)
  obsx[ti] = splinterp(obstime[ti], time, x)
  obsy[ti] = splinterp(obstime[ti], time, y)
  obsz[ti] = splinterp(obstime[ti], time, z)
  '''
  # Using numpy.interp
  # Zeros at endpoints, bad, write wrapper
  obsx[ti] = np.interp(obstime[ti], time, x)
  obsy[ti] = np.interp(obstime[ti], time, y)
  obsz[ti] = np.interp(obstime[ti], time, z)
...
def jdutc2jdtdb(jd_utc, taifile='/Users/bob/Documents/PhD/hstscan/hstscan/src/tai-utc.dat'):
    ...
    #print 'updated', uday, umonth, uyear
    #print 'today', pday, pmonth, pyear
    ...
    with open(taifile, 'r') as g:
        ...
        line = lines[-1].split()
        dt = float(line[6])
        assert line[5] == 'TAI-UTC=', 'Line format not as expected.'
    # TAI -> TT (terrestrial time) offset
    dt += 32.184 #s

    # TT -> TDB (solar system einstein delay) < 3.4 ms correction
    # not implemented yet
    return dt
